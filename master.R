...
setwd("F:\\antimicrobial resistance\\Simon\\thorax2\\FES2014\\R_code")
...
sum(df$uniq)
sum(df$uniq[df$organism.name == "HAEMOPHILUS INFLUENZAE"])
round( (sum(df$uniq[df$organism.name == "HAEMOPHILUS INFLUENZAE"]) / sum(df$uniq) )*100,2)
sum(df$uniq[df$organism.name == "STAPHYLOCOCCUS AUREUS"])
round( (sum(df$uniq[df$organism.name == "STAPHYLOCOCCUS AUREUS"]) / sum(df$uniq) )*100,2)
sum(df$uniq[df$organism.name == "STREPTOCOCCUS PNEUMONIAE"])
round( (sum(df$uniq[df$organism.name == "STREPTOCOCCUS PNEUMONIAE"]) / sum(df$uniq) )*100,2)

#source("age_distribution.R", echo = FALSE)

#source("trend_isolates.R", echo = FALSE)
...
# fig 2 ####
source("trend_resistance.R", echo = FALSE)
...
# total number isolates tested against ampamox
sum(df$ampamox.tested[df$year == 2013], na.rm = TRUE)
# age group analysis ####
